...
def render_page():
    ...
    distribution_loss_page = st.Page(
        rf'{pwd}/app_pages/distribution_loss.py',
        title='Bulk Purchase by Month Page'
    )

    re_inference_page = st.Page(
        rf'{pwd}/app_pages/re_inference.py',
        title='Re-train Page'
    ) 
  
    re_train_page = st.Page(
        rf'{pwd}/app_pages/re_train.py',
        title='Re-train Page'
    ) 
    
    gen_by_month_page = st.Page(
        rf'{pwd}/app_pages/generation_by_month_tree.py',
        title='Generation by Month Page'
    )
    ...
    ##menu = ['Economic', 'TSIC'] 
    with st.sidebar:
        selected = option_menu('Main Menu', ['Electricity Demand', 'Electricity Supply', 'Re-inference', 'Re-train'],
    ...
    # with st.sidebar:
    #     selected = option_menu('Electricity Supply', ['Bulk Purchase by Month'],
    #                            icons=['🏠', '📈', '📊', '📉', '📈', '📊'], menu_icon="cast", default_index=0)
    if selected == 'Electricity Demand':
        selected = option_menu('', ['TSIC - EGAT', 'TSIC - MEA', 'TSIC - PEA', 'Economic Index'],
    ...
    elif selected == 'TSIC - EGAT':
        st.navigation([tsic_egat_page]).run()
    elif selected == 'TSIC - MEA':
        st.navigation([tsic_mea_page]).run()
    elif selected == 'TSIC - PEA':
        st.navigation([tsic_pea_page]).run()
    elif selected == 'Electricity Supply':
        selected = option_menu('', ['Bulk Purchase by Month', 'Generation by Month'],
                                default_index=0, orientation="horizontal",
                                styles={
                                    "container": {"padding": "5px", "background-color": "#fff", "display": "flex"},
                                    "icon": {"color": "orange", "font-size": "25px"},
                                    "nav-link": {
                                        "font-size": "16px",
                                        "text-align": "left",
    ...
    if selected == 'Bulk Purchase by Month':
        ## padding = 5px, background-color = #fff
        st.navigation([distribution_loss_page]).run()
    elif selected == 'Generation by Month':
        st.navigation([gen_by_month_page]).run()
    elif selected == 'Re-inference':
        selected = option_menu('', ['Re-inference'],
                                default_index=0, orientation="horizontal",
                                styles={
                                    "container": {"padding": "5px", "background-color": "#fff"},
                                    "icon": {"color": "orange", "font-size": "25px"},
                                    ...
                                    "nav-link-selected": {"background-color": "white", "color": "#FFAA00"}
                                }
                                )
        if selected == 'Re-inference':
            st.navigation([re_inference_page]).run()
    elif selected == 'Re-train':
        selected = option_menu('', ['Re-train'],
                                default_index=0, orientation="horizontal",
                                styles={
                                    "container": {"padding": "5px", "background-color": "#fff"},
                                    "icon": {"color": "orange", "font-size": "25px"},
        ...
        if selected == 'Re-train':
            st.navigation([re_train_page]).run()
